...
class RingBuffer():
    ...
    def __init__(self, length):
        ...
        #print(f"RingBuffer initialized with length {length}. Actual data size: {self.data.size}")
...
class Env2DCylinderModified(gym.Env):
    ...
    def __init__(self, path_root, geometry_params, flow_params, solver_params, output_params,
                 optimization_params, inspection_params, n_iter_make_ready=None, verbose=0, size_history=6000,
        ...
        if self.stack_obs == True:
            self.state_shape = self.nstack * self.state_shape
    ...
    def reset(self):
        ...
        if self.stack_obs == True:
            next_state = self.Obs_FS(self.nstack, self.nskip)
        #print('reset next_state')
        #print(next_state)
    ...
    def Obs_FS(self, N_stack, N_skip, probe_nums = 64, action_nums = 2):
        ...
        #past_voltage = np.empty(action_nums * N_stack)
        ...
        for i in range(N_stack) :
            ...
            index_value = -1 if i == 0 else -i * N_skip #if i = 0, take the current obseravtion
            
            
            
            # Obtain past pressure measurements
            past_pressure[i*probe_nums : i*probe_nums + probe_nums] = [self.history_parameters["probe_{}".format(num_sensors)].get_value(index_value) 
                                   for num_sensors in range(probe_nums)]


  
            past_action[i*action_nums : i*action_nums + action_nums] = [self.history_parameters["jet_{}".format(num_actions)].get_value(index_value) 
